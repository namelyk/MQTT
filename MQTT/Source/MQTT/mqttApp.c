...
void mqttAppMsgReceived(Mqtt *xmqtt, uint8_t *topic, uint8_t topicLen, uint8_t *data, uint32_t dataLen)   
{   
...
void mqttAppSend()   
{   
    uint8_t flag = mqttPublish(&mqtt, "/presence", "Hello, here is mbed!",20);   
}   
...
{   
    mqttPublish(&mqtt, topic, data,len);   
}   
   
void mqttAppDisconnect(void)   
{   
